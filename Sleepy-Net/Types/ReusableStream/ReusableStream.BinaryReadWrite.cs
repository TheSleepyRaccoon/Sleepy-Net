...
namespace Sleepy
{
    public partial class ReusableStream
    {
        ...
        public unsafe string ReadString(bool nullable = false, Encoding encoding = null)
        {
            int encodedSize = (int)ReadVarUInt();
            ...
            int maxChars = encoding.GetMaxCharCount(encodedSize);
            char* charBuffer = stackalloc char[maxChars];
        ...
        /// <param name="encoding">The encoding to use. If null, the ReusableStream's <see cref="DefaultEncoding"/> will be used.</param>
        public void Write(string s, bool nullable = false, Encoding encoding = null)
        {
            if (s == null)
            {
                if (!nullable)
            ...
            StringPrepInfo info = PrepareForStringWrite(s.Length, encoding);

            byte[] data = Data;
            if (info.NeedsExactCount)
            {
                int realSize = encoding.GetByteCount(s);
            ...
            int bytesWritten = encoding.GetBytes(s, 0, s.Length, data, info.StringPos);
        ...
        public void WriteString(char[] chars, int offset, int count, bool nullable, Encoding encoding = null)
        {
            ...
            // we actually have some characters to write
            StringPrepInfo info = PrepareForStringWrite(count, encoding);

            byte[] data = Data;
            if (info.NeedsExactCount)
            {
                int realSize = encoding.GetByteCount(chars, offset, count);
                if (realSize > data.Length - info.StringPos)
                {
                    data = Grow(info.StringPos + realSize);
                }
            ...
            // if we've gotten here, then the data buffer is large enough to hold the string. Time to perform the write.
            int bytesWritten = encoding.GetBytes(chars, offset, count, data, info.StringPos);
            WriteVarIntImpl((ulong)bytesWritten, info.VarIntByteCount, info.VarIntPos);
        ...
        public unsafe void WriteString(char* chars, int count, bool nullable, Encoding encoding = null)
        {
            ...
            if (info.NeedsExactCount)
            {
                int realSize = encoding.GetByteCount(chars, count);
                if (realSize > data.Length - info.StringPos)
                {
                    data = Grow(info.StringPos + realSize);
                }
        ...
        {
            StringPrepInfo info = default(StringPrepInfo);

            int maxSize = encoding.GetMaxByteCount(count);
            ...
            byte[] data = Data;
            int remainingInBuffer = data.Length - info.StringPos;

            if (maxSize > remainingInBuffer)
            {
                // there might not be enough room in the data buffer
                int? minSize = GetMinimumEncodedByteCount(encoding, count);
