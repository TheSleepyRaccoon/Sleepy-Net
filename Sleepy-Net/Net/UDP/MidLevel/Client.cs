...
namespace Sleepy.Net.UDP
{
    public class Client
    {
        ...
        public bool AutoSetupEncryption = true;
        ...
        public class DynamicRequest<T, V> : CustomYieldInstruction where T : IMessage where V : IMessage, new()
        {
            ...
            public override bool keepWaiting { get { if (timeout != -1 && timeSent + timeout < Time.unscaledTime) { IsValid = false; return false; } return !GotResponse; } }
        ...
        public void SendLarge<T, V>(T message, Action<V> Callback, byte[] Data = null) where T : IMessage where V : IMessage, new() => SendLarge(ref message, Callback, Data);
